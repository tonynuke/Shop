...
export class Client {
    ...
    /**
     * Finds items by ids.
     * @param body (optional) Items ids.
     * @return Success
     */
    findItemsByIds(body: string[] | undefined , cancelToken?: CancelToken | undefined): Promise<ItemDto[]> {
        let url_ = this.baseUrl + "/api/v1/items/get-by-ids";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.instance.request(options_).catch((_error: any) => {
            ...
            return this.processFindItemsByIds(_response);
        });
    }

    protected processFindItemsByIds(response: AxiosResponse): Promise<ItemDto[]> {
        const status = response.status;
        let _headers: any = {};
        if (response.headers && typeof response.headers === "object") {
            for (let k in response.headers) {
